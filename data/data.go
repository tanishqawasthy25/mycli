...
func CreateTable() {
	createTableSQL := `CREATE TABLE IF NOT EXISTS mywork(
	...
	statement.Exec()
	log.Println("mywork table created")
}

func InsertNote(word string, definition string, category string) {
	insertNoteSQL := `INSERT INTO mywork(word, definition, category) VALUES (?, ?, ?)`
	...
	log.Println("Inserted mywork note successfully")
}

func DisplayAllNotes() {
	row, err := db.Query("SELECT * FROM mywork ORDER BY word")
	if err != nil {
		log.Fatal(err)
	}
